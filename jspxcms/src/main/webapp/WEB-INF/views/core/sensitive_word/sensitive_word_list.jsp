...
<div class="c-bar margin-top5">
  <span class="c-position"><s:message code="sensitiveWord.management"/> - <s:message code="list"/></span>
...
<form action="list.do" method="get">
    ...
    <legend><s:message code="search"/></legend>
	  <label class="c-lab"><s:message code="sensitiveWord.name"/>: <input type="text" name="search_CONTAIN_name" value="${search_CONTAIN_name[0]}"/></label>
...
<div class="ls-bc-opt">
	...
	<shiro:hasPermission name="core:sensitive_word:edit">
...
<table id="pagedTable" border="0" cellpadding="0" cellspacing="0" class="ls-tb margin-top5">
  ...
  <tr class="ls_table_th">
    ...
    <th class="ls-th-sort"><span class="ls-sort" pagesort="name"><s:message code="sensitiveWord.name"/></span></th>
    <th class="ls-th-sort"><span class="ls-sort" pagesort="replacement"><s:message code="sensitiveWord.replacement"/></span></th>
    <th class="ls-th-sort"><span class="ls-sort" pagesort="status"><s:message code="sensitiveWord.status"/></span></th>
  </tr>
  </thead>
  <tbody>
  <c:forEach var="bean" varStatus="status" items="${pagedList.content}">
  <tr<shiro:hasPermission name="core:sensitive_word:edit"> ondblclick="location.href=$('#edit_opt_${bean.id}').attr('href');"</shiro:hasPermission>>
    ...
    <td><c:out value="${bean.replacement}"/></td>
    <td align="center"><c:if test="${bean.status==1}"><b></c:if><s:message code="sensitiveWord.status.${bean.status}"/><c:if test="${bean.status==1}"></b></c:if></td>
  </tr>
  </c:forEach>
  </tbody>
</table>
