...
.clear{clear:both;}
.div1{margin-top:150px;padding-top:5px;background:url('') no-repeat 430px top;}
.desc{float:left;height:180px;width:430px;padding-top:40px;text-align:center;color:#858585;font-size:28px;font-weight:bold;background:url('') no-repeat center 8px;}
...
<div class="center div1">
	...
	<form id="validForm" action="login.do" method="post" class="input_form">
		<p class="input_p">
			...
			<input type="text" id="username" name="username" value="<%=defUsername%>" class="input required"/>
			<div class="clear"></div>
		</p>
		<p class="input_p">
			<label for="password" class="label">密码:</label>
			<input type="password" id="password" name="password" value="<%=defPassword%>" class="input"/>
			<div class="clear"></div>
		</p>
		<c:if test="${sessionScope.shiroCaptchaRequired || GLOBAL.captchaErrors<=0}">
		<p class="input_p">
			<label for="captcha" class="label">验证码:</label>
			<input type="text" id="captcha" name="captcha" class="input {required:true,remote:{url:'${ctx}/captcha.servlet',type:'post'},messages:{remote:'验证码错误'}}" autocomplete="off"/>
			<div class="clear"></div>
		</p>
		<p class="input_p">
